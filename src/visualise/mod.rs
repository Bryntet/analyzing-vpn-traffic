...
fn collect_data_hash(data: Vec<MetadataWrapper>) -> DataHash<HashMap<u32, usize>> {
   dbg!("data loaded");
   let mut hash: HashMap<Encryption,Arc<Mutex<HashMap<DataCategory,HashMap<u32,usize>>>>> = HashMap::new();
   data.into_iter().for_each(|data|{{
      ...
      let bytes = data.all_packets.par_iter()
          .flat_map(|packet|{
             let packet_data: Vec<&BasePacket> = packet.into();
             packet_data.into_par_iter()
                 .map(|packet|{
                    packet.bytes
                 })
          }).collect::<Vec<_>>();
      ...
      bytes.par_iter().for_each(|bytes|{
         *amount_per_byte_size.lock().unwrap().entry(*bytes).or_default() += 1;
      });
   ...
   hash.into_iter().map(|(enc,mutex)|{
      (enc, Arc::try_unwrap(mutex).unwrap().into_inner().unwrap())
   }).collect()
}

pub fn collect_data_specific_encryption(encryption: Encryption) -> DataHash<HashMap<u32, usize>> {
   let data = DataCategory::iter().map(|category|get_some_data(encryption.clone(),category)).collect_vec();
   collect_data_hash(data)
}
